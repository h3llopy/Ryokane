            ...
            rpc.query(params, { async: true })
                ...
                .then(function(values) {
                    if (values) {
                        ...
                        _.each(values, function(lst) {
                            self.giftCarValues.push(lst.amount);
                        });
                    }
                });
            ...
            if (self.card_no) {
                ...
                if (exchange_card_no) {
                    var params = {
                        ...
                        method: "write",
                        args: [
                            [self.card_id], { 'card_no': this.$('#new_card_no').val() }
                        ],
                    }
                    rpc.query(params, { async: false })
    ...
    var ConfirmationCardPayment = PopupWidget.extend({
        ...
        click_confirm: function() {
            ...
            if (self.recharge_card) {
                ...
                var gift_order = {
                    'giftcard_card_no': self.options.giftcard_card_no,
